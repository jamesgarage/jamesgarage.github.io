...
Add one cached original radial bank primitive through `src/world.mjs`: one central vertex and three rings of 32 vertices each. The center fan contributes 32 triangles; the two ring connections contribute 64 each, for **97 indexed vertices and 160 triangles per bank**. This remains below the previous 168 triangles. The denser perimeter softens the visible corners from the initial 20-segment prototype; foreground quality takes priority over maximizing triangle savings. No underside is needed because the opaque water covers the outer submerged boundary.
...
1. The built banks have finite positions/normals/colors/UVs, intact smooth vertex colors after batching, and the intended 160-triangle primitive budget. Compare the final world triangle count with the pre-change baseline. Each bank's transformed footprint satisfies its specified envelope; separate builds remain deterministic.
