...
3. Compare **A:** conservative terrain with local coarse particles, and **B:** an isolated finer PBD/XPBD patch with the same bucket. If cohesive sand is the priority, replace B with a small MPM/cohesive experiment and test hold/cut/fracture. Do not implement two complete engines.
