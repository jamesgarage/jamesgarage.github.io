...
Follow-up release review confirms that **Unity 6.6.0f1 was released August 31, 2026**. Its notes include iOS queued-frame, background-touch and audio-interruption fixes, as well as making Burst part of the engine rather than a separate package. These are reasons to include the supported 6.6 line in the native-lab comparison and check any middleware's package assumptions; they are not a measured advantage for our game. Some fixes may also have reached LTS patches, so compare the exact builds rather than assuming an exclusive feature. [Unity 6.6.0f1 release notes](https://unity.com/releases/editor/whats-new/6000.6.0f1)

Frame-pacing settings also vary within the LTS line. For example, 6.3.15f1 documents an iOS Metal display-link setting and behavior changes under serious/critical thermal state. Record display-link settings and thermal transitions in the 20-minute test; do not classify a deliberate engine thermal fallback as an unexplained solver slowdown. This is a specific release-note example, not a claim that 6.3.15f1 is the latest or selected patch. [Unity 6.3.15f1 release notes](https://unity.com/releases/editor/whats-new/6000.3.15f1)
...
Burst 1.8.30 documentation makes an important distinction: for Unity 2022.3 and later, Burst can compile iOS code from Windows, macOS or Linux; building the resulting Xcode project still requires macOS/Xcode. An iOS target produces a static library. Thus CPU particle development and export preparation can happen on this Windows workstation after design approval, while the final signed build and performance validation use a Mac route. Inspect build logs to confirm that Burst was actually used: unsupported/missing tooling can produce a build without its optimizations. Pin the package or built-in engine version used rather than inferring it from an old setup guide. [Burst platform build support](https://docs.unity3d.com/Packages/com.unity.burst@1.8/manual/building-projects.html)
